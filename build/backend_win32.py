    ...
    for register_config in data.registered_configurations:
        ...
        for settings in config_only_settings:
            ...
            if settings.cache:
                ...
                helper.add_line('@if exist "' + str(p) + '" goto CacheCheck_' + settings.target_name)
                helper.add_line('@if not exist "' + str(p) + '" goto Build_' + settings.target_name)
                helper.add_spacing()
                helper.add_line(":CacheCheck_" + settings.target_name)
                cache_mins = str(settings.max_cache_age_mins)
                helper.add_line("powershell -NoProfile -Command ^ \"if ((Get-Date) - (Get-Item '\"" + str(p) + "\"').LastWriteTime -gt [TimeSpan]::FromMinutes(" + cache_mins + ")) { exit 0 } else { exit 1 }\"")
                helper.add_spacing()
                helper.add_raw("if %ERRORLEVEL% NEQ 0 (\n")
                helper.print_space()
                helper.add_raw("    @echo [1m[93mTarget: " + settings.target_name +"[0m\n")
                helper.add_raw("    @echo [1m[93m~~~~~~~~~~~~~~~~~~~~~~[0m\n")
                helper.add_raw("    @echo [1mCache: [92mValid[0m\n")
                helper.add_raw("    goto Exit_" + settings.target_name + "\n")
                helper.add_raw(")")
                helper.add_spacing()
                helper.print_space()
                helper.print_line("[1m[93mTarget: " + settings.target_name +"[0m")
                helper.print_line("[1m[93m~~~~~~~~~~~~~~~~~~~~~~[0m")
                helper.print_line("[1mCache: [91mExpired[0m")
                helper.print_space()
                helper.add_spacing()

            helper.add_line(':Build_' + settings.target_name)
