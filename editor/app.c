...
pl_app_load(plApiRegistryI* ptApiRegistry, plAppData* ptAppData)
{
    ...
    *ptAppData->pbShowDeviceMemoryAnalyzer = gptConfig->load_bool("pbShowDeviceMemoryAnalyzer", *ptAppData->pbShowDeviceMemoryAnalyzer);
    ...
    plSceneInit tSceneInit = {
        .ptComponentLibrary = &ptAppData->tComponentLibrary
    };
    ...
    ptAppData->tMainCamera = gptEcs->create_perspective_camera(&ptAppData->tComponentLibrary, "main camera", pl_create_vec3(-4.7f, 4.2f, -3.256f), PL_PI_3, ptIO->tMainViewportSize.x / ptIO->tMainViewportSize.y, 0.1f, 48.0f, true, &ptMainCamera);
    gptCamera->set_pitch_yaw(ptMainCamera, 0.0f, 0.911f);
    gptCamera->update(ptMainCamera);
    gptEcs->attach_script(&ptAppData->tComponentLibrary, "pl_script_camera", PL_SCRIPT_FLAG_PLAYING | PL_SCRIPT_FLAG_RELOADABLE, ptAppData->tMainCamera, NULL);

    // create lights
    plLightComponent* ptLight = NULL;
    gptEcs->create_directional_light(&ptAppData->tComponentLibrary, "direction light", pl_create_vec3(-0.375f, -1.0f, -0.085f), &ptLight);
    ...
    plEntity tPointLight = gptEcs->create_point_light(&ptAppData->tComponentLibrary, "point light", pl_create_vec3(0.0f, 2.0f, 2.0f), &ptLight);
    ptLight->uShadowResolution = 1024;
    ptLight->tFlags |= PL_LIGHT_FLAG_CAST_SHADOW | PL_LIGHT_FLAG_VISUALIZER;
    plTransformComponent* ptPLightTransform = (plTransformComponent* )gptEcs->add_component(&ptAppData->tComponentLibrary, PL_COMPONENT_TYPE_TRANSFORM, tPointLight);
    ptPLightTransform->tTranslation = pl_create_vec3(0.0f, 1.497f, 2.0f);

    plEntity tSpotLight = gptEcs->create_spot_light(&ptAppData->tComponentLibrary, "spot light", pl_create_vec3(0.0f, 4.0f, -1.18f), pl_create_vec3(0.0, -1.0f, 0.376f), &ptLight);
    ptLight->uShadowResolution = 1024;
    ptLight->fRange = 5.0f;
    ptLight->fRadius = 0.025f;
    ptLight->fIntensity = 20.0f;
    ptLight->tFlags |= PL_LIGHT_FLAG_CAST_SHADOW | PL_LIGHT_FLAG_VISUALIZER;
    plTransformComponent* ptSLightTransform = (plTransformComponent* )gptEcs->add_component(&ptAppData->tComponentLibrary, PL_COMPONENT_TYPE_TRANSFORM, tSpotLight);
    ptSLightTransform->tTranslation = pl_create_vec3(0.0f, 4.0f, -1.18f);

    plEnvironmentProbeComponent* ptProbe = NULL;
    gptEcs->create_environment_probe(&ptAppData->tComponentLibrary, "Main Probe", pl_create_vec3(0.0f, 3.0f, 0.0f), &ptProbe);
    ...
    gptModelLoader->load_gltf(&ptAppData->tComponentLibrary, "../data/pilotlight-assets-master/models/gltf/humanoid/model.gltf", NULL, &tLoaderData0);
    gptModelLoader->load_gltf(&ptAppData->tComponentLibrary, "../data/pilotlight-assets-master/models/gltf/humanoid/floor.gltf", NULL, &tLoaderData0);
...
pl_app_resize(plWindow* ptWindow, plAppData* ptAppData)
{
    ...
    if(ptAppData->ptScene)
        gptCamera->set_aspect((plCameraComponent*)gptEcs->get_component(&ptAppData->tComponentLibrary, PL_COMPONENT_TYPE_CAMERA, ptAppData->tMainCamera), ptIO->tMainViewportSize.x / ptIO->tMainViewportSize.y);
...
pl_app_update(plAppData* ptAppData)
{
    ...
    *pdMemoryCounter = (double)gptMemory->get_memory_usage();
    ...
    plCameraComponent* ptCamera = (plCameraComponent*)gptEcs->get_component(&ptAppData->tComponentLibrary, PL_COMPONENT_TYPE_CAMERA, ptAppData->tMainCamera);
    ...
    gptEcs->run_script_update_system(&ptAppData->tComponentLibrary);
    gptEcs->run_animation_update_system(&ptAppData->tComponentLibrary, ptIO->fDeltaTime);
    gptPhysics->update(ptIO->fDeltaTime, &ptAppData->tComponentLibrary);
    gptEcs->run_transform_update_system(&ptAppData->tComponentLibrary);
    gptEcs->run_hierarchy_update_system(&ptAppData->tComponentLibrary);
    gptEcs->run_light_update_system(&ptAppData->tComponentLibrary);
    gptEcs->run_camera_update_system(&ptAppData->tComponentLibrary);
    gptEcs->run_inverse_kinematics_update_system(&ptAppData->tComponentLibrary);
    gptEcs->run_skin_update_system(&ptAppData->tComponentLibrary);
    gptEcs->run_object_update_system(&ptAppData->tComponentLibrary);
    gptEcs->run_environment_probe_update_system(&ptAppData->tComponentLibrary); // run after object update
    pl_end_cpu_sample(gptProfile, 0);
    ...
    if(gptRenderer->get_hovered_entity(ptAppData->ptView, &tNextEntity))
    {
        ...
        else if(ptAppData->tSelectedEntity.ulData != tNextEntity.ulData)
        {
            ...
            gptPhysics->set_angular_velocity(&ptAppData->tComponentLibrary, tNextEntity, pl_create_vec3(0, 0, 0));
            gptPhysics->set_linear_velocity(&ptAppData->tComponentLibrary, tNextEntity, pl_create_vec3(0, 0, 0));
        }
    ...
    {
        if(gptEcsTools->show_ecs_window(&ptAppData->tComponentLibrary, &ptAppData->tSelectedEntity, ptAppData->ptScene, &ptAppData->bShowEntityWindow))
        {
    ...
    if(ptAppData->tSelectedEntity.uIndex != UINT32_MAX)
    {
        ...
        plObjectComponent* ptSelectedObject = (plObjectComponent*)gptEcs->get_component(&ptAppData->tComponentLibrary, PL_COMPONENT_TYPE_OBJECT, ptAppData->tSelectedEntity);
        plTransformComponent* ptSelectedTransform = (plTransformComponent*)gptEcs->get_component(&ptAppData->tComponentLibrary, PL_COMPONENT_TYPE_TRANSFORM, ptAppData->tSelectedEntity);
        plTransformComponent* ptParentTransform = NULL;
        plHierarchyComponent* ptHierarchyComp = (plHierarchyComponent*)gptEcs->get_component(&ptAppData->tComponentLibrary, PL_COMPONENT_TYPE_HIERARCHY, ptAppData->tSelectedEntity);
        if(ptHierarchyComp)
        {
            ptParentTransform = (plTransformComponent*)gptEcs->get_component(&ptAppData->tComponentLibrary, PL_COMPONENT_TYPE_TRANSFORM, ptHierarchyComp->tParent);
        }
        ...
        {
            ptSelectedTransform = (plTransformComponent*)gptEcs->get_component(&ptAppData->tComponentLibrary, PL_COMPONENT_TYPE_TRANSFORM, ptSelectedObject->tTransform);
            gptGizmo->gizmo(ptGizmoDrawlist, ptCamera, ptSelectedTransform, ptParentTransform, (plVec2){0}, (plVec2){1.0f, 1.0f});
        }
    }
    ...
    if(ptAppData->bPhysicsDebugDraw)
    {
        ...
        gptPhysics->draw(&ptAppData->tComponentLibrary, ptDrawlist);
    }
