...
PL_API void pl_ecs_init_library   (plComponentLibrary*);
PL_API void pl_ecs_cleanup_library(plComponentLibrary*);

// memory owned by extension
PL_API plComponentLibrary* pl_ecs_create_library  (void);
PL_API void                pl_ecs_destroy_library(plComponentLibrary*);
...
PL_API bool pl_ecs_merge_library(plComponentLibrary* dest, plComponentLibrary* src);

// Creates new destination entities with new IDs.
// Clones components and fills source -> destination entity mapping.
PL_API bool pl_ecs_clone_library_into(plComponentLibrary* dest, plComponentLibrary* src, plHashMap64*);

// Associates extension-specific data with a component type in this library.
// The ECS does not own or free this pointer.
PL_API void  pl_ecs_set_library_type_data(plComponentLibrary*, plEcsTypeKey, void*, plEcsLibraryDataCleanup);
...
PL_API bool       pl_ecs_is_entity_valid      (const plComponentLibrary*, plEntity);
...
PL_API plEntityId pl_ecs_get_entity_id        (const plComponentLibrary*, plEntity);
PL_API plEntity   pl_ecs_get_entity_by_id     (const plComponentLibrary*, plEntityId);
...
PL_API void pl_ecs_get_entities(const plComponentLibrary*, plEntity*, uint32_t*);

// components
PL_API void*    pl_ecs_add_component (plComponentLibrary*, plEcsTypeKey, plEntity); // do not store
PL_API void*    pl_ecs_get_component (const plComponentLibrary*, plEcsTypeKey, plEntity); // do not store
PL_API bool     pl_ecs_has_component (const plComponentLibrary*, plEcsTypeKey, plEntity);
PL_API size_t   pl_ecs_get_index     (const plComponentLibrary*, plEcsTypeKey, plEntity);
PL_API uint32_t pl_ecs_get_components(const plComponentLibrary*, plEcsTypeKey, void**, const plEntity**); // do not store
...
// asset
PL_API void           pl_ecs_register_asset_types(void);
PL_API plAssetTypeKey pl_ecs_get_asset_type_key(void);

//----------------------------CORE COMPONENTS----------------------------------

// component types (can store)
PL_API plEcsTypeKey pl_ecs_get_ecs_type_key_tag    (void);
PL_API plEcsTypeKey pl_ecs_get_ecs_type_key_library(void);
