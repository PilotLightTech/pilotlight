...
pl_create_buffer(plDevice* ptDevice, const plBufferDesc* ptDesc, plBuffer** ptBufferOut)
{
    ...
    pl_set_vulkan_object_name(ptDevice, (uint64_t)tVulkanBuffer.tBuffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT, ptBuffer->tDesc.pcDebugName);
...
pl_create_swapchain_texture_view(plDevice* ptDevice, const plTextureViewDesc* ptViewDesc, VkImage tImage, const char* pcName)
{
    ...
    plTexture* ptTextureView = pl__get_texture(ptDevice, tHandle);
    ptTextureView->tView = *ptViewDesc;
    ...
    VkImageViewCreateInfo tViewInfo = {
        ...
        .subresourceRange.levelCount     = ptTextureView->tView.uMips,
...
pl_create_texture_view(plDevice* ptDevice, const plTextureViewDesc* ptViewDesc)
{
    ...
    const VkImageViewCreateInfo tViewInfo = {
        ...
        .format                          = pl__vulkan_format(ptViewDesc->tFormat),
        .subresourceRange.baseMipLevel   = ptViewDesc->uBaseMip,
        .subresourceRange.levelCount     = ptViewDesc->uMips == 0 ? ptOriginalTexture->tDesc.uMips - ptViewDesc->uBaseMip : ptViewDesc->uMips,
        .subresourceRange.baseArrayLayer = ptViewDesc->uBaseLayer,
        .subresourceRange.layerCount     = ptViewDesc->uLayerCount,
...
pl_create_compute_shader(plDevice* ptDevice, const plComputeShaderDesc* ptDescription)
{
    ...
    // if(ptDescription->pcShaderEntryFunc == NULL)
        ptShader->tDescription.tShader.pcEntryFunc = "main";
    ...
    for(uint32_t i = 0; i < ptShader->tDescription.uBindGroupLayoutCount; i++)
    {
        pl_create_bind_group_layout(ptDevice, &ptShader->tDescription.atBindGroupLayouts[i], "compute shader template bind group layout");
        ptVulkanShader->atDescriptorSetLayouts[i] = ptDevice->sbtBindGroupLayouts[ptShader->tDescription.atBindGroupLayouts[i]._uHandle].tDescriptorSetLayout;
    }
    ptVulkanShader->atDescriptorSetLayouts[ptShader->tDescription.uBindGroupLayoutCount]  = ptDevice->tDynamicDescriptorSetLayout;

    VkShaderModuleCreateInfo tShaderCreateInfo = {
        .sType    = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
        .codeSize = ptShader->tDescription.tShader.szCodeSize,
        .pCode    = (const uint32_t*)ptShader->tDescription.tShader.puCode
    };
    ...
    VkPipelineShaderStageCreateInfo tShaderStage = {
        ...
        .pName               = ptShader->tDescription.tShader.pcEntryFunc,
...
pl_create_shader(plDevice* ptDevice, const plShaderDescription* ptDescription)
{
    ...
    // if(ptDescription->pcPixelShaderEntryFunc == NULL)
        ptShader->tDescription.tPixelShader.pcEntryFunc = "main";

    // if(ptDescription->pcVertexShaderEntryFunc == NULL)
        ptShader->tDescription.tVertexShader.pcEntryFunc = "main";

    for(uint32_t i = 0; i < ptShader->tDescription.uBindGroupLayoutCount; i++)
    {
        pl_create_bind_group_layout(ptDevice, &ptShader->tDescription.atBindGroupLayouts[i], "shader template bind group layout");
        ptVulkanShader->atDescriptorSetLayouts[i] = ptDevice->sbtBindGroupLayouts[ptShader->tDescription.atBindGroupLayouts[i]._uHandle].tDescriptorSetLayout;
    }
    ptVulkanShader->atDescriptorSetLayouts[ptShader->tDescription.uBindGroupLayoutCount]  = ptDevice->tDynamicDescriptorSetLayout;
    ...
    VkShaderModuleCreateInfo tVertexShaderCreateInfo = {
        .sType    = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
        .codeSize = ptShader->tDescription.tVertexShader.szCodeSize,
        .pCode    = (const uint32_t*)(ptShader->tDescription.tVertexShader.puCode)
    };
    ...
    if(ptShader->tDescription.tPixelShader.puCode)
    {
        ...
        VkShaderModuleCreateInfo tPixelShaderCreateInfo = {
            .sType    = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
            .codeSize = ptShader->tDescription.tPixelShader.szCodeSize,
            .pCode    = (const uint32_t*)(ptShader->tDescription.tPixelShader.puCode),
        };
    ...
    const plRenderPassLayout* ptRenderPassLayout = pl_get_render_pass_layout(ptDevice, ptDescription->tRenderPassLayout);
    ...
    VkPipelineShaderStageCreateInfo tVertShaderStageInfo = {
        ...
        .pName  = ptShader->tDescription.tVertexShader.pcEntryFunc,
        .pSpecializationInfo = ptShader->tDescription.uConstantCount > 0 ? &tSpecializationInfo : NULL
    };
    ...
    VkPipelineShaderStageCreateInfo tFragShaderStageInfo = {
        ...
        .pName  = ptShader->tDescription.tPixelShader.pcEntryFunc,
    ...
    VkGraphicsPipelineCreateInfo tPipelineInfo = {
        ...
        .renderPass          = ptDevice->sbtRenderPassLayoutsHot[ptShader->tDescription.tRenderPassLayout.uIndex].tRenderPass,
...
pl_create_main_render_pass_layout(plSwapchain* ptSwap)
{
    ...
    plRenderPassLayout tLayout = {
        ...
        ._uGeneration = tHandle.uGeneration
    };
...
pl_create_main_render_pass(plSwapchain* ptSwap)
{
    ...
    ptPass->tDesc = (plRenderPassDescription){
        .tDimensions = gptIOI->get_io()->tMainViewportSize,
        .tLayout = ptDevice->tMainRenderPassLayout,
        .ptSwapchain = ptSwap
    };

    plRenderPassLayout* ptLayout = pl_get_render_pass_layout(ptDevice, ptDevice->tMainRenderPassLayout);
...
pl_create_render_pass_layout(plDevice* ptDevice, const plRenderPassLayoutDescription* ptDesc)
{
    ...
    pl__fill_common_render_pass_data(ptDesc, ptLayout, &tCommonData);
...
pl__vulkan_layout(plTextureUsage tUsage)
{
    switch(tUsage)
    {
        case PL_TEXTURE_USAGE_UNSPECIFIED:              return VK_IMAGE_LAYOUT_UNDEFINED;
