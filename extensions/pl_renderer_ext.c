...
plEcsTypeKey
pl_renderer_get_type_key_object(void)
{
    return gptData->tObjectComponentType;
}

plEcsTypeKey
pl_renderer_get_type_key_light(void)
{
    return gptData->tLightComponentType;
}

plEcsTypeKey
pl_renderer_get_type_key_environment_probe(void)
{
    return gptData->tEnvironmentProbeComponentType;
}

plEcsTypeKey
pl_renderer_get_type_key_environment(void)
{
    return gptData->tEnvironmentComponentType;
}

plEcsTypeKey
pl_renderer_get_type_key_renderer(void)
{
    return gptData->tRendererComponentType;
}

plEcsTypeKey
pl_renderer_get_type_key_terrain(void)
{
...
pl_renderer_initialize(const plRendererSettings* ptSettings)
{
    ...
    {
        gptConsole->add_toggle_variable("t.Renderer", &gptData->bShowTools, "shows renderer tools", PL_CONSOLE_VARIABLE_FLAGS_CLOSE_CONSOLE);
    }
...
pl_renderer_create_scene(const plSceneDesc* ptInit)
{
    ...
    const plDrawable tDrawable = {
        ...
        .uTransformIndex = pl__renderer_get_transform_slot(ptScene),
    ...
    pl_renderer_update_scene_asset(ptScene, ptMesh->atSubmeshes->tMaterial);
    return ptScene;
}
...
void
pl_renderer_set_scene_flags(plScene* ptScene, plSceneFlags tFlags)
{
    if(ptScene)
...
void
pl_renderer_load_component_library(plScene* ptScene, plComponentLibrary* ptLibrary)
{

    ptScene->ptComponentLibrary = ptLibrary;

    const plEntity* ptRendererEntities = NULL;
    uint32_t uRendererCount = gptEcs->get_components(ptLibrary, gptData->tRendererComponentType, NULL, &ptRendererEntities);
    
    if(uRendererCount > 0)
    {
        plRendererComponent* ptRendererComp = gptEcs->get_component(ptLibrary, gptData->tRendererComponentType, ptRendererEntities[0]);

        if(ptRendererComp)
            ptScene->tSettings = ptRendererComp->tRenderer;
        else
            ptScene->tSettings = gptAsset->load("/assets/settings/default.plrenderer");
    }
    else
    ...
    const plEntity* ptEnvironmentEntities = NULL;
    uint32_t uEnvironmentCount = gptEcs->get_components(ptLibrary, gptData->tEnvironmentComponentType, NULL, &ptEnvironmentEntities);
    
    if(uEnvironmentCount > 0)
    {
        plEnvironmentComponent* ptEnvironmentComp = gptEcs->get_component(ptLibrary, gptData->tEnvironmentComponentType, ptRendererEntities[0]);

        if(ptEnvironmentComp)
            ptScene->tEnvironment = ptEnvironmentComp->tEnvironment;
        else
            ptScene->tEnvironment = gptAsset->load("/assets/environments/skybox.plenvironment");
    }
    else
    ...
    ptScene->tFlags |= PL_RENDERER_SCENE_FLAGS_SKYBOX_DIRTY;
    ptScene->tFlags |= PL_RENDERER_SCENE_FLAGS_ALL_PROBES_DIRTY;

    const plEntity* ptProbeEntities = NULL;
    uint32_t uProbeCount = gptEcs->get_components(ptLibrary, gptData->tEnvironmentProbeComponentType, NULL, &ptProbeEntities);
    pl__renderer_add_probes_to_scene(ptScene, uProbeCount, ptProbeEntities);

    const plEntity* ptLightEntities = NULL;
    uint32_t uLightCount = gptEcs->get_components(ptLibrary, gptData->tLightComponentType, NULL, &ptLightEntities);
    pl__renderer_add_lights_to_scene(ptScene, uLightCount, ptLightEntities);

    const plEntity* ptObjectEntities = NULL;
    uint32_t uObjectCount = gptEcs->get_components(ptLibrary, gptData->tObjectComponentType, NULL, &ptObjectEntities);
    pl__renderer_add_drawable_objects_to_scene(ptScene, uObjectCount, ptObjectEntities);

    const plEntity* ptTerrainEntities = NULL;
    uint32_t uTerrainCount = gptEcs->get_components(ptLibrary, gptData->tTerrainComponentType, NULL, &ptTerrainEntities);
    if(uTerrainCount > 0)
    {
        plCommandBuffer* ptCmdBuffer = gptStarter->get_temporary_command_buffer();

        plTerrainComponent* ptTerrainComp = gptEcs->get_component(ptScene->ptComponentLibrary, gptData->tTerrainComponentType, ptTerrainEntities[0]);
        plTerrainAsset* ptTerrain = gptAsset->get_data(ptTerrainComp->tTerrain);
        ptScene->tTerrain = ptTerrainEntities[0];
...
pl_renderer_destroy_scene(plScene* ptScene)
{
    ...
    {
        pl__renderer_terrain_destroy(ptScene->ptTerrain);
    }
    ...
    pl_sb_free(ptScene->sbuActiveDrawables);
    ...
    pl_sb_free(ptScene->sbuFreeTransformIndices)
    pl_sb_free(ptScene->sbuFreeSkinSlots)
    pl_sb_free(ptScene->sbuFreeProbeDataSlots)
    pl_sb_free(ptScene->sbuFreeDrawableSlots)
...
pl_renderer_create_view(plScene* ptScene, const plViewDesc* ptDesc)
{
    ...
    ptView->bFrustumCulling = true;

    // default editor options
    ptView->uOutlineWidth = 4;
    ptView->bShowSelectedBoundingBox = true;
    ptView->fGridCellSize = 0.025f;
    ptView->fGridMinPixelsBetweenCells = 2.0f;
    ptView->tGridColorThin = (plVec4){0.5f, 0.5f, 0.5f, 1.0f};
    ptView->tGridColorThick = (plVec4){0.75f, 0.75f, 0.75f, 1.0f};
...
pl_renderer_prepare_scene(plScene* ptScene, const plCamera** atCameras, uint32_t uCameraCount)
{
    ...
    const uint32_t uObjectCount = pl_sb_size(ptScene->sbuActiveDrawables);
    ...
    if(ptSettings->tShadows.tFlags & PL_RENDERER_SHADOW_FLAGS_MULTI_VIEWPORT)
    {
        for(uint32_t i = 0; i < uObjectCount; i++)
        {
            ...
            memcpy(&ptTransformBuffer->tMemoryAllocation.pHostMapped[ptDrawable->uTransformIndex * sizeof(plMat4)], &ptTransform->tWorld, sizeof(plMat4));
            ...
            if(ptDrawable->uInstanceCount != 0)
            {
                ptDrawable->uInstanceIndex = uInstanceOffset;

                for(int32_t iViewport = 0; iViewport < 6; iViewport++)
                {
                    for(uint32_t uInstance = 0; uInstance < ptDrawable->uInstanceCount; uInstance++)
                    {
                        PL_ASSERT(uInstance == 0);
                        uint32_t uTransformIndex = ptScene->sbtDrawables[ptScene->sbuActiveDrawables[i] + uInstance].uTransformIndex;
...
void
pl_renderer_render_debug_view(plView* ptView, const plRenderViewDesc* ptViewDesc)
{
    PL_PROFILE_BEGIN_SAMPLE_API(gptProfile, 0, __FUNCTION__);
    ...
    if(ptView->bFreezeCameraNextFrame || !ptView->bFrozenCamera)
    {
        ptView->tCullCamera = *ptCamera;
        ptView->bFreezeCameraNextFrame = false;
    }

    const plCamera* ptCullCamera = ptView->bFrustumCulling ? &ptView->tCullCamera : NULL;

    // debug lights
    if(ptView->bDrawLightsDebug)
    {
        PL_PROFILE_BEGIN_SAMPLE_API(gptProfile, 0, "draw lights");
        ...
        {
            if(ptLights[i].tFlags & PL_LIGHT_FLAG_VISUALIZER)
            {
                const plVec4 tColor = {.rgb = ptLights[i].tColor, .a = 1.0f};
                if(ptLights[i].tType == PL_LIGHT_TYPE_POINT)
                {
                    plSphere tSphere = {
                        .fRadius = ptLights[i].fRadius,
                        .tCenter = ptLights[i].tPosition
                    };
                    gptDraw->add_3d_sphere(ptView->pt3DDrawList, tSphere, 6, 6, (plDrawLineOptions){.uColor = PL_COLOR_32_VEC4(tColor), .fThickness = 0.005f});
                    tSphere.fRadius = ptLights[i].fRange;
                    plSphere tSphere2 = {
                        .fRadius = ptLights[i].fRange,
                        .tCenter = ptLights[i].tPosition
                    };
                    gptDraw->add_3d_sphere(ptView->pt3DDrawList, tSphere2, 0, 0, (plDrawLineOptions){.uColor = PL_COLOR_32_VEC4(tColor), .fThickness = 0.01f});
                }
                else if(ptLights[i].tType == PL_LIGHT_TYPE_SPOT)
                {
                    plCone tCone0 = {
                        .fRadius = tanf(ptLights[i].fOuterConeAngle * 0.5f) * ptLights[i].fRange,
                        .tTipPos = ptLights[i].tPosition,
                        .tBasePos = pl_add_vec3(ptLights[i].tPosition, pl_mul_vec3_scalarf(ptLights[i].tDirection, ptLights[i].fRange))
                    };
                    gptDraw->add_3d_cone(ptView->pt3DDrawList, tCone0, 0, (plDrawLineOptions){.uColor = PL_COLOR_32_VEC4(tColor), .fThickness = 0.01f});

                    if(ptLights[i].fInnerConeAngle > 0.0f)
                    {
                        plCone tCone1 = {
                            .fRadius = tanf(ptLights[i].fInnerConeAngle) * ptLights[i].fRange,
                            .tTipPos = ptLights[i].tPosition,
                            .tBasePos = pl_add_vec3(ptLights[i].tPosition, pl_mul_vec3_scalarf(ptLights[i].tDirection, ptLights[i].fRange))
                        };
                ...
                else if(ptLights[i].tType == PL_LIGHT_TYPE_DIRECTIONAL)
                {
                    plVec3 tDirection = pl_norm_vec3(ptLights[i].tDirection);
                    plCone tCone0 = {
                        .fRadius = 0.125f,
    ...
    if(ptView->bDrawBoundingBoxesDebug)
    {
        PL_PROFILE_BEGIN_SAMPLE_API(gptProfile, 0, "draw bounding boxes");
        const uint32_t uDrawableCount = pl_sb_size(ptScene->sbuActiveDrawables);
        for(uint32_t i = 0; i < uDrawableCount; i++)
        {
            plObjectComponent* ptObject = gptEcs->get_component(ptScene->ptComponentLibrary, gptData->tObjectComponentType, ptScene->sbtDrawableResources[ptScene->sbuActiveDrawables[i]].tEntity);
...
pl_renderer_render_view(plView* ptView, const plRenderViewDesc* ptViewDesc)
{
    ...
    if(ptScene->ptTerrain)
    {
        ...
        if(ptTerrainComp->tFlags & PL_TERRAIN_FLAGS_WIREFRAME)
...
pl_renderer_begin_frame(void)
{
    ...
    if(gptData->bShowTools)
    {
        ...
        if(pl_sb_size(gptData->sbptScenes) > 0 && gptUI->begin_window("Renderer", &gptData->bShowTools, 0))
        {
            ...
            for(uint32_t uSceneIndex = 0; uSceneIndex < pl_sb_size(gptData->sbptScenes); uSceneIndex++)
            {
                ...
                if(gptUI->tree_node_f("Scene: %u", 0, uSceneIndex))
                {
                    if(!(ptScene->tInternalFlags & PL_SCENE_INTERNAL_FLAG_ACTIVE))
                        continue;
                    ...
                    if(gptUI->button("Update Environment"))
                    {
                        ptScene->tFlags |= PL_RENDERER_SCENE_FLAGS_ALL_PROBES_DIRTY;
                    ...
                    for(uint32_t uViewIndex = 0; uViewIndex < pl_sb_size(ptScene->sbptViews); uViewIndex++)
                    {
                        ...
                        if(gptUI->tree_node_f("View: %u", 0, uViewIndex))
                        {
                            gptUI->checkbox("Show Origin", &ptView->bShowOrigin);
                            ...
                            if(gptUI->checkbox("Freeze Culling", &ptView->bFrozenCamera))
                            {
                                if(ptView->bFrozenCamera)
                                    ptView->bFreezeCameraNextFrame = true;
                            }
                            gptUI->slider_uint("Outline Width", &ptView->uOutlineWidth, 2, 50, 0);
                            gptUI->tree_pop();
                        }
                    }
    ...
    for(uint32_t uSceneIndex = 0; uSceneIndex < pl_sb_size(gptData->sbptScenes); uSceneIndex++)
    {
        ...
        plRenderEnvironment* ptEnvironment = gptAsset->get_data(ptScene->tEnvironment);
        if(ptScene->tFlags & PL_RENDERER_SCENE_FLAGS_SKY_LUTS_DIRTY)
        {
            pl__renderer_scene_create_sky_luts_textures(ptScene);
            pl__renderer_scene_update_sky_luts_bindgroups(ptScene);
        }

        if(ptEnvironment->eMode == PL_RENDERER_SKY_MODE_SKYBOX && ptScene->tFlags & PL_RENDERER_SCENE_FLAGS_SKYBOX_DIRTY)
        {
...
void
pl_renderer_update_scene_asset(plScene* ptScene, plAssetHandle tMaterial)
{
    plAssetTypeKey tType =  gptAsset->get_type_key(tMaterial);
    if(tType == gptMaterial->get_asset_type_key())
    {
        pl_sb_push(ptScene->sbtDirtyMaterials, tMaterial);
    }
    else if(tType == gptData->tAssetTypeKeyEnvironment)
    {
        ptScene->tFlags |= PL_RENDERER_SCENE_FLAGS_ALL_PROBES_DIRTY;
        ptScene->tFlags |= PL_RENDERER_SCENE_FLAGS_SKY_LUTS_DIRTY;
    }
...
static void
pl__ecs_object_serialize(void* pComponent, const plComponentLibrary* ptLibrary, plEntityId tEntityId, plJsonObject* ptJson)
{
...
static void
pl__ecs_object_deserialize(plJsonObject* ptJson, plComponentLibrary* ptLibrary, plEntityId tEntityId, void* pComponent)
{
    plObjectComponent* ptComponent = pComponent;
...
static void
pl__ecs_object_resolve(plComponentLibrary* ptLibrary, plEntityId tEntityId, plHashMap64* ptHashmap, void* pComponent)
{
    plObjectComponent* ptComponent = pComponent;
    if(ptHashmap)
    {
        plEntityId tOldTransformID = ptComponent->tTransformId;
        ptComponent->tTransformId = pl_hm_lookup(ptHashmap, tOldTransformID);
    }
    
    ptComponent->tTransform = gptEcs->get_entity_by_id(ptLibrary, ptComponent->tTransformId);
}

static void
pl__ecs_light_serialize(void* pComponent, const plComponentLibrary* ptLibrary, plEntityId tEntityId, plJsonObject* ptJson)
{
    ...
    gptJson->add_float_array(ptJson, "color", ptComponent->tColor.d, 3);
}

static void
pl__ecs_light_deserialize(plJsonObject* ptJson, plComponentLibrary* ptLibrary, plEntityId tEntityId, void* pComponent)
{
    plLightComponent* ptComponent = pComponent;
...
static void
pl__ecs_environment_serialize(void* pComponent, const plComponentLibrary* ptLibrary, plEntityId tEntityId, plJsonObject* ptJson)
{
    plEnvironmentComponent* ptComponent = pComponent;

    gptJson->add_string_member(ptJson, "environment", gptAsset->get_path(ptComponent->tEnvironment));
}

static void
pl__ecs_environment_deserialize(plJsonObject* ptJson, plComponentLibrary* ptLibrary, plEntityId tEntityId, void* pComponent)
{
    plEnvironmentComponent* ptComponent = pComponent;

    char acTempBuffer0[1024] = {0};
    gptJson->string_member(ptJson, "environment", acTempBuffer0, 1024);
    ptComponent->tEnvironment = gptAsset->load(acTempBuffer0);
}

static void
pl__ecs_renderer_serialize(void* pComponent, const plComponentLibrary* ptLibrary, plEntityId tEntityId, plJsonObject* ptJson)
{
    plRendererComponent* ptComponent = pComponent;

    gptJson->add_string_member(ptJson, "renderer", gptAsset->get_path(ptComponent->tRenderer));
}

static void
pl__ecs_renderer_deserialize(plJsonObject* ptJson, plComponentLibrary* ptLibrary, plEntityId tEntityId, void* pComponent)
{
    plRendererComponent* ptComponent = pComponent;

    char acTempBuffer0[1024] = {0};
    gptJson->string_member(ptJson, "renderer", acTempBuffer0, 1024);
    ptComponent->tRenderer = gptAsset->load(acTempBuffer0);
}

static void
pl__ecs_probe_serialize(void* pComponent, const plComponentLibrary* ptLibrary, plEntityId tEntityId, plJsonObject* ptJson)
{
...
static void
pl__ecs_probe_deserialize(plJsonObject* ptJson, plComponentLibrary* ptLibrary, plEntityId tEntityId, void* pComponent)
{
    plEnvironmentProbeComponent* ptComponent = pComponent;
...
static void
pl__ecs_terrain_serialize(void* pComponent, const plComponentLibrary* ptLibrary, plEntityId tEntityId, plJsonObject* ptJson)
{
    plTerrainComponent* ptComponent = pComponent;
    gptJson->add_string_member(ptJson, "terrain", gptAsset->get_path(ptComponent->tTerrain));
    gptJson->add_float_member(ptJson, "tau", ptComponent->fTau);
    gptJson->add_float_member(ptJson, "slope_start", ptComponent->fSlopeStart);
    gptJson->add_float_member(ptJson, "slope_end", ptComponent->fSlopeEnd);
    gptJson->add_float_member(ptJson, "depth_bias", ptComponent->fTerrainShadowConstantDepthBias);
    gptJson->add_float_member(ptJson, "slope_depth_bias", ptComponent->fTerrainShadowSlopeDepthBias);
    gptJson->add_bool_member(ptJson, "wireframe", ptComponent->tFlags & PL_TERRAIN_FLAGS_WIREFRAME);
    gptJson->add_bool_member(ptJson, "show_levels", ptComponent->tFlags & PL_TERRAIN_FLAGS_SHOW_LEVELS);
}

static void
pl__ecs_terrain_deserialize(plJsonObject* ptJson, plComponentLibrary* ptLibrary, plEntityId tEntityId, void* pComponent)
{
    plTerrainComponent* ptComponent = pComponent;
    char acTempBuffer0[1024] = {0};
    gptJson->string_member(ptJson, "terrain", acTempBuffer0, 1024);
    ptComponent->fTau = gptJson->float_member(ptJson, "tau", 0.2f);
    ptComponent->fSlopeStart = gptJson->float_member(ptJson, "slope_start", 0.0f);
    ptComponent->fSlopeEnd = gptJson->float_member(ptJson, "slope_end", 0.45f);
    ptComponent->fTerrainShadowConstantDepthBias = gptJson->float_member(ptJson, "depth_bias", -100.0f);
    ptComponent->fTerrainShadowSlopeDepthBias = gptJson->float_member(ptJson, "slope_depth_bias", -10.0f);
    if(gptJson->bool_member(ptJson, "wireframe", false)) ptComponent->tFlags |= PL_TERRAIN_FLAGS_WIREFRAME;
    if(gptJson->bool_member(ptJson, "show_levels", false)) ptComponent->tFlags |= PL_TERRAIN_FLAGS_SHOW_LEVELS;

    ptComponent->tTerrain = gptAsset->load(acTempBuffer0);
    plTerrainAsset* ptTerrain = gptAsset->get_data(ptComponent->tTerrain);
    gptTerrain->process(ptTerrain);
}

void
pl_renderer_register_ecs_components(void)
{
    ...
    const plComponentDesc tObjectDesc = {
        ...
        .resolve = pl__ecs_object_resolve,
    };
    ...
    const plComponentDesc tEnvironmentDesc = {
        .pcName = "environment",
        .szSize = sizeof(plEnvironmentComponent),
        .serialize = pl__ecs_environment_serialize,
        .deserialize = pl__ecs_environment_deserialize,
    };
    gptData->tEnvironmentComponentType = gptEcs->register_type(tEnvironmentDesc, NULL);

    const plComponentDesc tRendererDesc = {
        .pcName = "renderer",
        .szSize = sizeof(plRendererComponent),
        .serialize = pl__ecs_renderer_serialize,
        .deserialize = pl__ecs_renderer_deserialize,
    };
    gptData->tRendererComponentType = gptEcs->register_type(tRendererDesc, NULL);
}
        ...
        if(pl_hm_has_key(&ptScene->tDrawableHashmap, uMainDrawHash))
        {
            ...
            for(uint32_t uSubmeshIndex = 0; uSubmeshIndex < ptMainDrawableResources->uSubmeshCount; uSubmeshIndex++)
            {
                ...
                if(pl_hm_has_key(&ptScene->tDrawableHashmap, uDrawHash))
                {
                    ...
                    uint64_t uDrawableIndex = pl_hm_lookup(&ptScene->tDrawableHashmap, uDrawHash);
                    plDrawable* ptDrawable = &ptScene->sbtDrawables[uDrawableIndex];
                    plDrawableResources* ptDrawableResources = &ptScene->sbtDrawableResources[uDrawableIndex];

                    if(ptDrawableResources->uSkinIndex != UINT32_MAX)
                    {
                        gptFreeList->return_node(&ptScene->tSkinBufferFreeList, ptScene->sbtSkinData[ptDrawableResources->uSkinIndex].ptFreeListNode);
                        ptScene->sbtSkinData[ptDrawableResources->uSkinIndex].ptFreeListNode = NULL;
                    }

                    pl__renderer_return_transform_slot(ptScene, ptDrawable->uTransformIndex);
                    pl__renderer_return_skin_slot(ptScene, ptDrawableResources->uSkinIndex);
                    pl__renderer_return_drawable_slot(ptScene, (uint32_t)uDrawableIndex);
                    pl_hm_remove(&ptScene->tDrawableHashmap, uDrawHash);

                    if(ptDrawableResources->ptIndexBufferNode)  gptFreeList->return_node(&ptScene->tIndexBufferFreeList, ptDrawableResources->ptIndexBufferNode);
                    if(ptDrawableResources->ptVertexBufferNode) gptFreeList->return_node(&ptScene->tVertexBufferFreeList, ptDrawableResources->ptVertexBufferNode);
                    if(ptDrawableResources->ptDataBufferNode)   gptFreeList->return_node(&ptScene->tStorageBufferFreeList, ptDrawableResources->ptDataBufferNode);
                    if(ptDrawableResources->ptSkinBufferNode)   gptFreeList->return_node(&ptScene->tStorageBufferFreeList, ptDrawableResources->ptSkinBufferNode);

                    ptDrawableResources->ptIndexBufferNode = NULL;  
                    ptDrawableResources->ptVertexBufferNode = NULL; 
                    ptDrawableResources->ptDataBufferNode = NULL;   
                    ptDrawableResources->ptSkinBufferNode = NULL;   
                    ptDrawableResources->uSkinIndex = UINT32_MAX;
                    ptDrawableResources->uSubmeshIndex = 0;
    ...
    // check if light
    {
        ...
        for(uint32_t i = 0; i < uPointLightCount; i++)
        {
            if(ptScene->sbtPointLights[i].tEntity.uData == tObject.uData)
            {
                ...
                pl_sb_del_swap(ptScene->sbtPointLights, i);
                bFound = true;
                break;
            }
        }
        ...
        for(uint32_t i = 0; i < uSpotLightCount; i++)
        {
            if(ptScene->sbtSpotLights[i].tEntity.uData == tObject.uData)
            {
                ...
                pl_sb_del_swap(ptScene->sbtSpotLights, i);
                bFound = true;
                break;
            }
        }
        ...
        for(uint32_t i = 0; i < uDirLightCount; i++)
        {
            if(ptScene->sbtDirectionLights[i].tEntity.uData == tObject.uData)
            {
                ...
                pl_sb_del_swap(ptScene->sbtDirectionLights, i);
                bFound = true;
                break;
            }
        }
        ...
        for(uint32_t i = 0; i < uProbeCount; i++)
        {
            ...
            if(ptProbe->tEntity.uData == tObject.uData)
            {
                ...
                for(uint32_t j = 0; j < 6; j++)
                    gptGfx->queue_texture_for_deletion(gptData->ptDevice, ptProbe->atRawOutputTextureViews[j]);

                gptGfx->queue_buffer_for_deletion(gptData->ptDevice, ptProbe->tViewBuffer);
                gptGfx->queue_buffer_for_deletion(gptData->ptDevice, ptProbe->tSunCameraBuffers);
                ...
                gptGfx->queue_bind_group_for_deletion(gptData->ptDevice, ptProbe->tViewBG);
                gptGfx->queue_bind_group_for_deletion(gptData->ptDevice, ptProbe->tGBufferBG);
                gptGfx->queue_bind_group_for_deletion(gptData->ptDevice, ptProbe->tDShadowBG);
                gptGfx->queue_bind_group_for_deletion(gptData->ptDevice, ptProbe->tSunCameraBG);
                
                pl_sb_del_swap(ptScene->sbtProbeData, i);
                bFound = true;
                break;
            }
        }
...
pl_load_renderer_ext(plApiRegistryI* ptApiRegistry, bool bReload)
{
    ...
    tApi0.render_debug_view              = pl_renderer_render_debug_view;
    ...
    tApi0.load_component_library         = pl_renderer_load_component_library;
    ...
    tApi0.update_scene_asset             = pl_renderer_update_scene_asset;
    ...
    tApi0.get_ecs_type_key_environment        = pl_renderer_get_type_key_environment;
    tApi0.get_ecs_type_key_renderer           = pl_renderer_get_type_key_renderer;
    tApi0.run_light_update_system             = pl_renderer_run_light_update_system;
    tApi0.run_environment_probe_update_system = pl_renderer_run_environment_probe_update_system;
    tApi0.get_ecs_type_key_object             = pl_renderer_get_type_key_object;
    tApi0.run_object_update_system            = pl_renderer_run_object_update_system;
    tApi0.register_ecs_components             = pl_renderer_register_ecs_components;
    tApi0.get_drawlist                       = pl_renderer_debug_get_drawlist;
