...
pl_refr_cleanup(void)
{
    ...
    for(uint32_t i = 0; i < pl_sb_size(gptData->sbtScenes); i++)
    {
        ...
        pl_sb_free(ptScene->sbtNewDeferredDrawables);
        pl_sb_free(ptScene->sbtNewForwardDrawables);
...
pl_refr_run_ecs(uint32_t uSceneHandle)
{
    ...
    gptECS->run_environment_probe_update_system(&ptScene->tComponentLibrary);
...
pl_refr_render_scene(uint32_t uSceneHandle, const uint32_t* auViewHandles, const plViewOptions* atOptions, uint32_t uViewCount)
{
    ...
    for(uint32_t uViewIndex = 0; uViewIndex < uViewCount; uViewIndex++)
    {
        ...
        const uint32_t uDeferredDrawableCount = pl_sb_size(ptScene->sbtDeferredDrawables);
        const uint32_t uForwardDrawableCount = pl_sb_size(ptScene->sbtForwardDrawables);
        ...
        if(ptCullCamera)
        {
            ...
            gptJob->dispatch_batch(uDeferredDrawableCount, 0, tDeferredJobDesc, &ptDeferredCounter);
            ...
            gptJob->dispatch_batch(uForwardDrawableCount, 0, tForwardJobDesc, &ptForwardCounter);
        }
        else // no culling, just copy drawables over
        {
            if(pl_sb_size(ptView->sbtVisibleOpaqueDrawables) != uDeferredDrawableCount)
            {
                pl_sb_resize(ptView->sbtVisibleOpaqueDrawables, uDeferredDrawableCount);
                memcpy(ptView->sbtVisibleOpaqueDrawables, ptScene->sbtDeferredDrawables, sizeof(plDrawable) * uDeferredDrawableCount);
            }
            if(pl_sb_size(ptView->sbtVisibleTransparentDrawables) != uForwardDrawableCount)
            {
                pl_sb_resize(ptView->sbtVisibleTransparentDrawables, uForwardDrawableCount);
                memcpy(ptView->sbtVisibleTransparentDrawables, ptScene->sbtForwardDrawables, sizeof(plDrawable) * uForwardDrawableCount);
            }
        }
        ...
        if(ptCullCamera)
        {
            ...
            for(uint32_t uDrawableIndex = 0; uDrawableIndex < uDeferredDrawableCount; uDrawableIndex++)
            {
        ...
        *gptData->pdDrawCalls += (double)uVisibleDeferredDrawCount;
        ...
        *gptData->pdDrawCalls += 1.0;
        ...
        if(ptCullCamera)
        {
            ...
            for(uint32_t uDrawableIndex = 0; uDrawableIndex < uForwardDrawableCount; uDrawableIndex++)
            {
        ...
        *gptData->pdDrawCalls += (double)uVisibleForwardDrawCount;
        for(uint32_t i = 0; i < uVisibleForwardDrawCount; i++)
        {
            ...
            ptDynamicData->tModel = ptTransform->tWorld;
            ptDynamicData->iMaterialOffset = tDrawable.uMaterialIndex;
            ptDynamicData->uGGXEnvSampler        = ptScene->sbtProbeData[0].uGGXEnvSampler;
            ptDynamicData->uGGXLUT               = ptScene->sbtProbeData[0].uGGXLUT;
            ptDynamicData->uLambertianEnvSampler = ptScene->sbtProbeData[0].uLambertianEnvSampler;
            ptDynamicData->uGlobalIndex = 0;
        ...
        {
            for(uint32_t i = 0; i < uDeferredDrawableCount; i++)
            {
            ...
            for(uint32_t i = 0; i < uForwardDrawableCount; i++)
            {
        ...
        *gptData->pdDrawCalls += 1.0;
        ...
        for(uint32_t i = 0; i < uJumpSteps; i++)
        {
            ...
            gptGfx->pipeline_barrier_compute(ptJumpEncoder, PL_STAGE_VERTEX | PL_STAGE_COMPUTE, PL_ACCESS_SHADER_READ, PL_STAGE_COMPUTE, PL_ACCESS_SHADER_WRITE);
            ...
            // end render pass
            gptGfx->pipeline_barrier_compute(ptJumpEncoder, PL_STAGE_COMPUTE, PL_ACCESS_SHADER_WRITE, PL_STAGE_VERTEX | PL_STAGE_COMPUTE | PL_STAGE_PIXEL, PL_ACCESS_SHADER_READ);
...
pl_add_drawable_objects_to_scene(uint32_t uSceneHandle, uint32_t uDeferredCount, const plEntity* atDeferredObjects, uint32_t uForwardCount, const plEntity* atForwardObjects)
{
    ...
    #if 1
    const uint32_t uTransparentStart = pl_sb_size(ptScene->sbtNewForwardDrawables);
    pl_sb_add_n(ptScene->sbtNewForwardDrawables, uForwardCount);

    const uint32_t uOpaqueStart = pl_sb_size(ptScene->sbtNewDeferredDrawables);
    pl_sb_add_n(ptScene->sbtNewDeferredDrawables, uDeferredCount);

    for(uint32_t i = 0; i < uDeferredCount; i++)
        ptScene->sbtNewDeferredDrawables[uOpaqueStart + i].tEntity = atDeferredObjects[i];

    for(uint32_t i = 0; i < uForwardCount; i++)
        ptScene->sbtNewForwardDrawables[uTransparentStart + i].tEntity = atForwardObjects[i];
    #endif

    #if 0 // send through forward pass only
    const uint32_t uTransparentStart = pl_sb_size(ptScene->sbtNewForwardDrawables);
    pl_sb_add_n(ptScene->sbtNewForwardDrawables, uForwardCount + uDeferredCount);

    for(uint32_t i = 0; i < uDeferredCount; i++)
        ptScene->sbtNewForwardDrawables[uTransparentStart + i].tEntity = atDeferredObjects[i];

    for(uint32_t i = 0; i < uForwardCount; i++)
        ptScene->sbtNewForwardDrawables[uDeferredCount + uTransparentStart + i].tEntity = atForwardObjects[i];
    #endif

    #if 0 // send through deferred pass only
    const uint32_t uTransparentStart = pl_sb_size(ptScene->sbtNewDeferredDrawables);
    pl_sb_add_n(ptScene->sbtNewDeferredDrawables, uForwardCount + uDeferredCount);

    for(uint32_t i = 0; i < uDeferredCount; i++)
        ptScene->sbtNewDeferredDrawables[uTransparentStart + i].tEntity = atDeferredObjects[i];

    for(uint32_t i = 0; i < uForwardCount; i++)
        ptScene->sbtNewDeferredDrawables[uDeferredCount + uTransparentStart + i].tEntity = atForwardObjects[i];
    #endif
}
