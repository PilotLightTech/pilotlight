...
pl_renderer_create_scene(plSceneInit tInit)
{
    ...
    plEntity tMaterial = gptECS->create_material(ptScene->ptComponentLibrary, "environment probe material", &ptMaterial);
    ...
    ptScene->tProbeMesh = gptECS->create_sphere_mesh(ptScene->ptComponentLibrary, "environment probe mesh", 0.25f, 32, 32, &ptMesh);
...
pl_renderer_outline_entities(plScene* ptScene, uint32_t uCount, plEntity* atEntities)
{
    ...
    for(uint32_t i = 0; i < uOldSelectedEntityCount; i++)
    {
        ...
        plObjectComponent*   ptObject   = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_OBJECT, tEntity);
        plMeshComponent*     ptMesh     = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_MESH, ptObject->tMesh);
        plMaterialComponent* ptMaterial = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_MATERIAL, ptMesh->tMaterial);
...
pl_renderer_prepare_scene(plScene* ptScene)
{
    ...
    for(uint32_t i = 0; i < uObjectCount; i++)
    {
        ...
        plObjectComponent* ptObject = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_OBJECT, ptScene->sbtDrawables[i].tEntity);

        // copy transform into proper location in CPU side buffer
        plTransformComponent* ptTransform = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_TRANSFORM, ptObject->tTransform);
    ...
    for(uint32_t i = 0; i < uSkinCount; i++)
    {
        ...
        plSkinComponent* ptSkinComponent = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_SKIN, ptScene->sbtSkinData[i].tEntity);
    ...
    for(uint32_t uProbeIndex = 0; uProbeIndex < uProbeCount; uProbeIndex++)
    {
        ...
        plEnvironmentProbeComponent* ptProbeComp = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_ENVIRONMENT_PROBE, ptProbe->tEntity);
        ...
        plTransformComponent* ptProbeTransform = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_TRANSFORM, ptProbe->tEntity);
    ...
    {
        plEnvironmentProbeComponent* ptProbe = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_ENVIRONMENT_PROBE, ptScene->sbtProbeData[i].tEntity);
        plObjectComponent* ptObject = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_OBJECT, ptScene->sbtProbeData[i].tEntity);
        plTransformComponent* ptProbeTransform = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_TRANSFORM, ptScene->sbtProbeData[i].tEntity);
...
void
pl_renderer_prepare_view(plView* ptView, plCameraComponent* ptCamera)
{
    pl_begin_cpu_sample(gptProfile, 0, __FUNCTION__);
    ...
    if(gptData->tRuntimeOptions.bShowProbes)
    {
        ...
        for(uint32_t uProbeIndex = 0; uProbeIndex < uProbeCount; uProbeIndex++)
        {
            plEnvironmentProbeData* ptProbe = &ptScene->sbtProbeData[uProbeIndex];
            plEnvironmentProbeComponent* ptProbeComp = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_ENVIRONMENT_PROBE, ptProbe->tEntity);
            plObjectComponent* ptObject = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_OBJECT, ptProbe->tEntity);
...
void
pl_renderer_render_view(plView* ptView, plCameraComponent* ptCamera, plCameraComponent* ptCullCamera)
{
    pl_begin_cpu_sample(gptProfile, 0, __FUNCTION__);
    ...
    *gptData->pdDrawCalls += (double)(pl_sb_size(ptView->sbtVisibleOpaqueDrawables) + pl_sb_size(ptView->sbtVisibleTransparentDrawables) + 1);
    ...
    for(uint32_t i = 0; i < uVisibleDeferredDrawCount; i++)
    {
        ...
        {
            plObjectComponent* ptObject = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_OBJECT, tDrawable.tEntity);
            plTransformComponent* ptTransform = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_TRANSFORM, ptObject->tTransform);
    ...
    for(uint32_t uSceneIndex = 0; uSceneIndex < pl_sb_size(gptData->sbptScenes); uSceneIndex++)
    {
        ...
        for(uint32_t uViewIndex = 0; uViewIndex < pl_sb_size(ptScene->sbptViews); uViewIndex++)
        {
            ...
            if(ptView->auHoverResultProcessing[uFrameIdx])
            {
                ...
                plEntity tNewEntity = {
                    .uIndex = uNewID,
                    .uGeneration = ptScene->ptComponentLibrary->sbtEntityGenerations[uNewID]
                };
...
pl_renderer_add_drawable_objects_to_scene(plScene* ptScene, uint32_t uObjectCount, const plEntity* atObjects)
{
    ...
    for(uint32_t i = 0; i < uObjectCount; i++)
    {
        ...
        plObjectComponent* ptObject = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_OBJECT, atObjects[i]);
        plMeshComponent* ptMesh = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_MESH, ptObject->tMesh);
        plMaterialComponent* ptMaterial = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_MATERIAL, ptMesh->tMaterial);
    ...
    for (uint32_t i = 0; i < uObjectCount - 1; i++)
    {
        ...
        {
            plObjectComponent* ptObjectA = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_OBJECT, ptScene->sbtStagedDrawables[uStart + j].tEntity);
            plObjectComponent* ptObjectB = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_OBJECT, ptScene->sbtStagedDrawables[uStart + j + 1].tEntity);
    ...
    for (uint32_t i = 0; i < uObjectCount; i++)
    {
        ...
        plObjectComponent* ptObjectA = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_OBJECT, ptScene->sbtStagedDrawables[uStart + i].tEntity);
...
pl_renderer_update_scene_materials(plScene* ptScene, uint32_t uMaterialCount, const plEntity* atMaterials)
{
    ...
    for(uint32_t i = 0; i < uMaterialCount; i++)
    {
        ...
        plMaterialComponent* ptMaterial = gptECS->get_component(ptScene->ptComponentLibrary, PL_COMPONENT_TYPE_MATERIAL, tMaterialComp);
