...
pl__renderer_cull_job(plInvocationData tInvoData, void* pData, void* pGroupSharedMemory)
{
    ...
    if(ptObject->tFlags & PL_OBJECT_FLAGS_RENDERABLE)
    {
        ...
        {
            plConvexPoly tFrustumPoly = { .pVertices = ptCullData->atFrustumCorners, .iVertexCount = 8 };
            if(gptGjk->pen(pl_gjk_support_convex_poly, &tFrustumPoly, pl_gjk_support_aabb, &ptObject->tAABB, NULL))
            {
