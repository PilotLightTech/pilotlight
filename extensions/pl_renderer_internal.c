...
pl__renderer_cull_job(plInvocationData tInvoData, void* pData, void* pGroupSharedMemory)
{
    ...
    plRendererScene* ptScene = ptCullData->ptScene;
    ...
    plMesh* ptMesh = gptAsset->get_mesh(ptObject->tMesh);
...
static void
pl__renderer_perform_skinning(plCommandBuffer* ptCommandBuffer, plRendererScene* ptScene)
{
...
static bool
pl__renderer_pack_shadow_atlas(plRendererScene* ptScene)
{
    PL_PROFILE_BEGIN_SAMPLE_API(gptProfile, 0, __FUNCTION__);
...
static bool
pl__renderer_pack_view_shadow_atlas(plRendererScene* ptScene)
{
    PL_PROFILE_BEGIN_SAMPLE_API(gptProfile, 0, __FUNCTION__);
    ...
    const plPackRect tSunPackRect = {
        .iWidth  = (int)ptScene->tSkyOptions.uShadowResolution * (int)ptScene->tSkyOptions.uShadowCascadeCount,
        .iHeight = (int)ptScene->tSkyOptions.uShadowResolution,
        .iId     = 0
    };
...
static void
pl__renderer_generate_shadow_maps(plCommandBuffer* ptCommandBuffer, plRendererScene* ptScene, const plCamera** atCameras, uint32_t uCameraCount)
{
    PL_PROFILE_BEGIN_SAMPLE_API(gptProfile, 0, __FUNCTION__);
...
static void
pl__renderer_generate_direction_view_map(plCommandBuffer* ptCommandBuffer, plRendererScene* ptScene, const plCamera* ptSceneCamera, plCSMInfo tInfo)
{
    PL_PROFILE_BEGIN_SAMPLE_API(gptProfile, 0, __FUNCTION__);
    ...
    for(uint32_t uRectIndex = 1; uRectIndex < uAtlasRectCount; uRectIndex++)
    {
        ...
        if(ptScene->ptTerrain)
        {
            ...
            for(uint32_t i = 0; i < pl_sb_size(ptScene->ptTerrain->sbtChunkFiles); i++)
                pl__render_chunk_shadow(ptScene, ptScene->ptTerrain, &tShadowCamera, ptCommandBuffer, &ptScene->ptTerrain->sbtChunkFiles[i].tFile.atChunks[0], &ptScene->ptTerrain->sbtChunkFiles[i].tFile, 0, (float)ptLight->uShadowResolution, (float)ptLight->uShadowResolution);
        }
...
static void
pl__renderer_generate_sun_shadow_map(plCommandBuffer* ptCommandBuffer, plRendererScene* ptScene, plGpuViewData* ptViewData, plBufferHandle tViewBuffer, plBufferHandle tSunCameraBuffer, const plCamera* ptSceneCamera, plBindGroupHandle tBindGroup, bool bProbe)
{
    PL_PROFILE_BEGIN_SAMPLE_API(gptProfile, 0, __FUNCTION__);
    ...
    // for(uint32_t uRectIndex = 0; uRectIndex < uAtlasRectCount; uRectIndex++)
    {
        ...
        for(uint32_t uCascade = 0; uCascade < uCascadeCount; uCascade++)
        {
            ...
            const float fReceiverMinZ = fZMin - fCenterZ;
            const float fReceiverMaxZ = fZMax - fCenterZ;

            const float fNearZ = fReceiverMinZ - fDepthPadding;
            const float fFarZ  = fReceiverMaxZ + fDepthPadding;
            ...
            if(ptScene->ptTerrain)
            {
                ...
                for(uint32_t i = 0; i < pl_sb_size(ptScene->ptTerrain->sbtChunkFiles); i++)
                    pl__render_chunk_shadow(ptScene, ptScene->ptTerrain, &tShadowCamera, ptCommandBuffer, &ptScene->ptTerrain->sbtChunkFiles[i].tFile.atChunks[0], &ptScene->ptTerrain->sbtChunkFiles[i].tFile, 0, (float)uShadowResolution, (float)uShadowResolution);
            }
        }
    }
...
static uint32_t
pl__renderer_probe_data_pack_index(plRendererScene* ptScene, uint32_t uResolution)
{
...
static uint64_t
pl__renderer_add_material_to_scene(plRendererScene* ptScene, plAssetHandle tMaterial)
{
    plMaterial* ptMaterial = gptAsset->get_material(tMaterial);
    ...
    if(!pl_hm_has_key(&ptScene->tMaterialHashmap, tMaterial.uData))
    {
        ...
        for(uint32_t uTextureIndex = 0; uTextureIndex < PL_TEXTURE_SLOT_COUNT; uTextureIndex++)
        {
            ...
            {
                const char* pcSourceFile = gptAsset->get_source_file(ptMaterial->atTextures[uTextureIndex].tTexture);
                plResourceHandle tTextureResource = gptResource->load(pcSourceFile, PL_RESOURCE_LOAD_FLAG_BLOCK_COMPRESSED);
...
static bool
pl__renderer_add_drawable_data_to_global_buffer(plRendererScene* ptScene, uint32_t uDrawableIndex, uint32_t uSubmeshIndex)
{
    ...
    plMesh* ptMainMesh = gptAsset->get_mesh(ptObject->tMesh);
    ...
    if(ptMesh->ptVertexWeights[1]) { uSkinStride += 1; uVertexStreamMask |= PL_MESH_FORMAT_FLAG_HAS_WEIGHTS_1; }
    ...
    if(ptObject->tSkinComponent.uIndex != UINT32_MAX)
    ...
    if(ptSkinVertexDataBufferNode == NULL && ptObject->tSkinComponent.uIndex != UINT32_MAX)
    {
        bResizeNeeded = true;
    }
    ...
    if(gptECS->is_entity_valid(ptScene->ptComponentLibrary, ptObject->tSkinComponent))
    {
        ...
        plSkinData tSkinData = {
            .tEntity           = ptObject->tSkinComponent,
        ...
        plSkinComponent* ptSkinComponent = gptECS->get_component(ptScene->ptComponentLibrary, gptData->tSkinComponentType, ptObject->tSkinComponent);
...
static uint32_t
pl__renderer_get_bindless_texture_index(plRendererScene* ptScene, plTextureHandle tTexture)
{
...
static uint32_t
pl__renderer_get_bindless_cube_texture_index(plRendererScene* ptScene, plTextureHandle tTexture)
{
    uint64_t uIndex = 0;
...
static void
pl__renderer_return_bindless_texture_index(plRendererScene* ptScene, plTextureHandle tTexture)
{
    uint64_t uIndex = 0;
    if(pl_hm_has_key_ex(&ptScene->tTextureIndexHashmap, tTexture.uData, &uIndex))
    {
...
static void
pl__renderer_return_bindless_cube_texture_index(plRendererScene* ptScene, plTextureHandle tTexture)
{
    uint64_t uIndex = 0;
    if(pl_hm_has_key_ex(&ptScene->tCubeTextureIndexHashmap, tTexture.uData, &uIndex))
    {
...
static void
pl__renderer_probe_create_environment_map(plRendererScene* ptScene, plEnvironmentProbeData* ptProbe)
{
    PL_PROFILE_BEGIN_SAMPLE_API(gptProfile, 0, __FUNCTION__);
    plDevice* ptDevice = gptData->ptDevice;
...
static void
pl__render_view_gbuffer_fill_pass(plRendererScene* ptScene, plCommandBuffer* ptCommandBuffer, plGbufferFillPassInfo* ptInfo)
{
    // for convience
    const uint32_t uFrameIdx = gptGfx->get_current_frame_index();
    plDevice*      ptDevice  = gptData->ptDevice;
...
static void
pl__render_view_deferred_lighting_pass(plRendererScene* ptScene, plCommandBuffer* ptCommandBuffer, plBindGroupHandle tViewBG, plDeferredLightingPassInfo* ptInfo)
{
...
pl__render_view_deferred_lighting_debug_pass(plView* ptView, plCommandBuffer* ptCommandBuffer, plBindGroupHandle tViewBG)
{
    ...
    plDevice*      ptDevice  = gptData->ptDevice;
    plDrawStream*  ptStream  = &gptData->tDrawStream;
    plRendererScene*       ptScene   = ptView->ptParentScene;
...
static void
pl__render_view_skybox_pass(plRendererScene* ptScene, plCommandBuffer* ptCommandBuffer, plBindGroupHandle tViewBG, plMat4* ptTransform, plDrawArea* ptArea, uint32_t uFace)
{
...
static void
pl__render_view_forward_pass(plRendererScene* ptScene, plCommandBuffer* ptCommandBuffer, plBindGroupHandle tViewBG, plForwardPassInfo* ptInfo)
{
    const uint32_t uFrameIdx = gptGfx->get_current_frame_index();
    plDevice*      ptDevice  = gptData->ptDevice;
    plDrawStream*  ptStream  = &gptData->tDrawStream;
...
pl__render_view_debug_pass(plView* ptView, const plCamera* ptCamera, const plCamera* ptCullCamera)
{
    ...
    if(ptScene->tSkyOptions.tFlags & PL_RENDERER_SKY_FLAGS_SHOW_VISUALIZER)
    {
        const plVec4 tColor = {.rgb = ptScene->tSkyOptions.tSunColor, .a = 1.0f};
        plVec3 tDirection = pl_norm_vec3(ptScene->tSkyOptions.tSunDirection);
        plCone tCone0 = {
            .fRadius = 0.125f,
...
static void
pl__render_view_aerial_pass(plView* ptView)
{
    plCommandPool* ptCmdPool = gptStarter->get_current_command_pool();
    plDevice*      ptDevice  = gptData->ptDevice;
    plRendererScene*       ptScene   = ptView->ptParentScene;
    ...
    gptGfx->begin_compute_pass(ptPostCmdBuffer, &tResources);
    gptGfx->push_debug_group(ptPostCmdBuffer, "Aerial Compute", (plVec4){0.0f, 0.32f, 0.10f, 1.0f});
    
    plDynamicBinding tDynamicBinding = pl__allocate_dynamic_data(ptDevice, sizeof(uint32_t));
    uint32_t* ptDynamicData = (uint32_t*)tDynamicBinding.pcData;
    *ptDynamicData = 0; // camera index

    const uint32_t uFrameIdx = gptGfx->get_current_frame_index();

    gptGfx->bind_compute_shader(ptPostCmdBuffer, ptScene->tSkyAerialShader);
    plBindGroupHandle atPostBGs[] = {ptScene->atSceneBindGroups[uFrameIdx], ptView->atViewBG[uFrameIdx], ptView->atSkyAerialBG2[uFrameIdx] };
    gptGfx->bind_compute_bind_groups(ptPostCmdBuffer, ptScene->tSkyAerialShader, 0, PL_ARRAYSIZE(atPostBGs), atPostBGs, 1, &tDynamicBinding);
...
static void
pl__renderer_probe_update_all(plRendererScene* ptScene)
{
    ...
    for(uint32_t uProbeIndex = 0; uProbeIndex < uProbeCount; uProbeIndex++)
    {
        ...
        for(uint32_t uFaceIndex = 0; uFaceIndex < 6; uFaceIndex++)
        {
            ...
            if(ptScene->tSkyOptions.tMode & PL_RENDERER_SKY_MODE_REALISTIC)
            {
                ...
                plBindGroupHandle atSkyLutBGs[] = {ptScene->atSceneBindGroups[uFrameIdx], ptProbe->tViewBG, ptScene->tSkyProbeLutBG2 };
           ...
           .tRenderArea = {
            ...
            if(ptProbeComp->tFlags & PL_ENVIRONMENT_PROBE_FLAGS_INCLUDE_SKY)
            {
                ...
                else if(ptScene->tSkyOptions.tMode == PL_RENDERER_SKY_MODE_REALISTIC)
                {
                    ...
                    uint32_t* ptDynamicData = (uint32_t*)tDynamicBinding.pcData;
                    *ptDynamicData = uFace;
                    gptGfx->bind_shader(ptCmdBuffer, ptScene->tSkyShader);
                    plBindGroupHandle atSkyBGs[] = {ptScene->atSceneBindGroups[uFrameIdx], ptProbe->tViewBG, ptScene->tSkyProbeBG2 };
                    gptGfx->bind_graphics_bind_groups(ptCmdBuffer, ptScene->tSkyShader, 0, PL_ARRAYSIZE(atSkyBGs), atSkyBGs, 1, &tDynamicBinding);
                    plDraw tDraw = {
                        .uInstanceCount = 1,
                        .uVertexCount = 3
                    };
...
static void
pl__renderer_scene_create_bindgroups(plRendererScene* ptScene)
{
    ...
    ptScene->tSkyProbeBG2 = gptGfx->create_bind_group(gptData->ptDevice, &tSkyBG2Desc);
...
static void
pl__renderer_scene_update_sky_luts_bindgroups(plRendererScene* ptScene)
{
    for(uint32_t i = 0; i < gptGfx->get_frames_in_flight(); i++)
    {
    ...
    const plBindGroupUpdateData tBGData3 = {
        .atTextureBindings = {
            {.uSlot = 0, .eType = PL_TEXTURE_BINDING_TYPE_SAMPLED, .tTexture = ptScene->tSkyProbeLut},
            {.uSlot = 1, .eType = PL_TEXTURE_BINDING_TYPE_SAMPLED, .tTexture = ptScene->atSunTransmissionLut[0]}
        }
    };
    gptGfx->update_bind_group(gptData->ptDevice, ptScene->tSkyProbeBG2, &tBGData3);

    const plBindGroupUpdateData tBGSkyLut = {
        .atTextureBindings = {
            {.uSlot = 0, .eType = PL_TEXTURE_BINDING_TYPE_STORAGE, .tTexture = ptScene->tSkyProbeLut},
            {.uSlot = 1, .eType = PL_TEXTURE_BINDING_TYPE_SAMPLED, .tTexture = ptScene->atSunTransmissionLut[0]},
            {.uSlot = 2, .eType = PL_TEXTURE_BINDING_TYPE_SAMPLED, .tTexture = ptScene->atSunMultiscatterLut[0]}
        }
    };
    gptGfx->update_bind_group(gptData->ptDevice, ptScene->tSkyProbeLutBG2, &tBGSkyLut);
}

static void
pl__renderer_scene_update_bindgroups(plRendererScene* ptScene)
{
...
static void
pl__renderer_scene_create_sky_luts_textures(plRendererScene* ptScene)
{
    plDevice* ptDevice = gptData->ptDevice;
    if(gptGfx->is_texture_valid(ptDevice, ptScene->atSunTransmissionLut[0]))
    {
        gptGfx->queue_texture_for_deletion(ptDevice, ptScene->tSkyProbeLut);
        for(uint32_t i = 0; i < gptGfx->get_frames_in_flight(); i++)
        {
            gptGfx->queue_texture_for_deletion(ptDevice, ptScene->atSunTransmissionLut[i]);
            gptGfx->queue_texture_for_deletion(ptDevice, ptScene->atSunMultiscatterLut[i]);
        }
    }

    const plTextureDesc tSunTransmissionLutDesc = {
        .tDimensions = { .xy = ptScene->tSkyOptions.tTransmissionLutResolution, 1},
    ...
    const plTextureDesc tSunMultiscatterLutDesc = {
        .tDimensions = { .xy = ptScene->tSkyOptions.tMultiscatterLutResolution, 1},
        .eFormat     = PL_FORMAT_R11G11B10_FLOAT,
        .uLayers     = 1,
        .uMips       = 1,
        .eType       = PL_TEXTURE_TYPE_2D,
    ...
    const plTextureDesc tSkyLutDesc = {
        .tDimensions = { .xy = ptScene->tSkyOptions.tSkyLutResolution, 1},
        .eFormat     = PL_FORMAT_R11G11B10_FLOAT,
        .uLayers     = 1,
        .uMips       = 1,
        .eType       = PL_TEXTURE_TYPE_2D,
    ...
    gptStarter->create_texture(&tSkyLutDesc, NULL, 0, &ptScene->tSkyProbeLut);
    for(uint32_t i = 0; i < gptGfx->get_frames_in_flight(); i++)
    {
...
static void
pl__renderer_scene_create_textures(plRendererScene* ptScene)
{
    // shadow atlas
...
static void
pl__renderer_scene_create_buffers(plRendererScene* ptScene)
{
...
static void
pl__renderer_scene_create_brdf_lut(plRendererScene* ptScene)
{
...
static void
pl__renderer_view_create_sky_luts_textures(plView* ptView)
{
    plDevice* ptDevice = gptData->ptDevice;
    ...
    if(gptGfx->is_texture_valid(ptDevice, ptView->atSkyLut[0]))
    {
        for(uint32_t i = 0; i < gptGfx->get_frames_in_flight(); i++)
        {
            gptGfx->queue_texture_for_deletion(ptDevice, ptView->atSkyLut[i]);
            gptGfx->queue_texture_for_deletion(ptDevice, ptView->atSkyAerialLut[i]);
        }
    }

    const plTextureDesc tSkyLutDesc = {
        .tDimensions = { .xy = ptView->ptParentScene->tSkyOptions.tSkyLutResolution, 1},
        .eFormat     = PL_FORMAT_R11G11B10_FLOAT,
        .uLayers     = 1,
        .uMips       = 1,
        .eType       = PL_TEXTURE_TYPE_2D,
    ...
    const plTextureDesc tAerialLutDesc = {
        .tDimensions = ptView->ptParentScene->tSkyOptions.tAerialLutResolution,
        .eFormat     = PL_FORMAT_R16G16B16A16_FLOAT,
        .uLayers     = 1,
        .uMips       = 1,
        .eType       = PL_TEXTURE_TYPE_3D,
        .eUsage      = PL_TEXTURE_USAGE_STORAGE | PL_TEXTURE_USAGE_SAMPLED,
        .pcDebugName = "tAerialLut"
    };

    for(uint32_t i = 0; i < gptGfx->get_frames_in_flight(); i++)
    {
        gptStarter->create_texture(&tSkyLutDesc, NULL, 0, &ptView->atSkyLut[i]);
        gptStarter->create_texture(&tAerialLutDesc, NULL, 0, &ptView->atSkyAerialLut[i]);
    }
}
...
pl__renderer_view_create_bindgroups(plView* ptView)
{
    ...
    for(uint32_t i = 0; i < gptGfx->get_frames_in_flight(); i++)
    {
        ...
        ptView->atSkyAerialLutBG2[i] = gptGfx->create_bind_group(gptData->ptDevice, &tSkyAerialLutBG2Desc);
        ptView->atSkyBG2[i] = gptGfx->create_bind_group(gptData->ptDevice, &tSkyBG2Desc);
        ptView->atSkyAerialBG2[i] = gptGfx->create_bind_group(gptData->ptDevice, &tSkyAerialBG2Desc);
    }
}
...
pl__renderer_view_update_sky_luts_bindgroups(plView* ptView)
{
    ...
    for(uint32_t i = 0; i < gptGfx->get_frames_in_flight(); i++)
    {
        ...
        const plBindGroupUpdateData tBGData4 = {
            .atTextureBindings = {
                {.uSlot = 0, .eType = PL_TEXTURE_BINDING_TYPE_STORAGE, .tTexture = ptView->atSkyAerialLut[i]},
                {.uSlot = 1, .eType = PL_TEXTURE_BINDING_TYPE_SAMPLED, .tTexture = ptScene->atSunTransmissionLut[i]},
                {.uSlot = 2, .eType = PL_TEXTURE_BINDING_TYPE_SAMPLED, .tTexture = ptScene->atSunMultiscatterLut[i]}
            }
        };
        gptGfx->update_bind_group(gptData->ptDevice, ptView->atSkyAerialLutBG2[i], &tBGData4);

        const plBindGroupUpdateData tBGData3 = {
            .atTextureBindings = {
                {.uSlot = 0, .eType = PL_TEXTURE_BINDING_TYPE_SAMPLED, .tTexture = ptView->atSkyLut[i]},
                {.uSlot = 1, .eType = PL_TEXTURE_BINDING_TYPE_SAMPLED, .tTexture = ptScene->atSunTransmissionLut[i]}
            }
        };
        gptGfx->update_bind_group(gptData->ptDevice, ptView->atSkyBG2[i], &tBGData3);

        const plBindGroupUpdateData tBGData6 = {
            .atTextureBindings = {
                {.uSlot = 0, .eType = PL_TEXTURE_BINDING_TYPE_STORAGE, .tTexture = ptView->tFinalTexture},
                {.uSlot = 1, .eType = PL_TEXTURE_BINDING_TYPE_SAMPLED, .tTexture = ptView->tDepthTexture},
                {.uSlot = 2, .eType = PL_TEXTURE_BINDING_TYPE_SAMPLED, .tTexture = ptView->atSkyAerialLut[i]}
            }
        };
        gptGfx->update_bind_group(gptData->ptDevice, ptView->atSkyAerialBG2[i], &tBGData6);
    }
}
...
static void
pl__renderer_probe_create_textures(plRendererScene* ptScene, plEnvironmentProbeData* ptProbeData)
{
...
static void
pl__renderer_probe_create_buffers(plRendererScene* ptScene, plEnvironmentProbeData* ptProbeData)
{
...
static void
pl__renderer_probe_create_bindgroups(plRendererScene* ptScene, plEnvironmentProbeData* ptProbeData)
{
    const plBindGroupDesc tGlobalBGDesc = {
        .ptPool      = gptData->ptBindGroupPool,
        .tLayout     = gptData->tShadowGlobalBGLayout,
...
static void
pl__renderer_probe_update_bindgroups(plRendererScene* ptScene, plEnvironmentProbeData* ptProbeData)
{
    const plBindGroupUpdateData tDShadowBGData = {
        .atBufferBindings = {
            {
...
void
pl__renderer_scene_load_skybox_from_panorama(plRendererScene* ptScene, const char* pcPath, int iResolution)
{
    PL_PROFILE_BEGIN_SAMPLE_API(gptProfile, 0, __FUNCTION__);
