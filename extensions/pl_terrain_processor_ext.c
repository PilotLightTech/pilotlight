...
pl__initialize_cdlod_heightmap(plTerrainHeightMap* ptHeightMap, uint32_t uTileCount, plTerrainTileInfo* atTiles, plTerrainTileInfo* atHaloTiles)
{
    ...
    uint16_t* auHaloHeightMapData = PL_ALLOC((4 * (ptHeightMap->iSize - 1) + 3) * sizeof(uint16_t));
    memset(auHaloHeightMapData, 0, (4 * (ptHeightMap->iSize - 1) + 3) * sizeof(uint16_t));

    for(uint32_t uTileIndex = 0; uTileIndex < 8; uTileIndex++)
    {
        ...
        else if(uTileIndex == 7) //  northwest
        {
            int x = iImageWidth - 1;
            int y = iImageHeight - 1;
            uint16_t uRawValue = *(uint16_t*)&pucImageData[(x + y * iImageWidth) * sizeof(uint16_t)]; // southeast corner
            auHaloHeightMapData[(ptHeightMap->iSize - 1) * 4 + 2] = uRawValue;
        }
...
pl__get_normal(plTerrainHeightMap* ptHeightMap, plTerrainMapElement* ptElement)
{
    ...
    // TODO: FIX IF ELLIPSOID FIX WORKS
    float hL = ptElement->iX == 0 ? ptHeightMap->atHaloElements[3 * (ptHeightMap->iSize - 1) + hz].fY : ptHeightMap->atElements[iL + ptHeightMap->iSize*ptElement->iZ].fY;
    float hR = ptElement->iX >= ptHeightMap->iSize - 1 ? ptHeightMap->atHaloElements[(ptHeightMap->iSize - 1) + hz].fY : ptHeightMap->atElements[iR + ptHeightMap->iSize*ptElement->iZ].fY;
    float hD = ptElement->iZ == 0 ? ptHeightMap->atHaloElements[hx].fY : ptHeightMap->atElements[ptElement->iX  + ptHeightMap->iSize*jD].fY;
    float hU = ptElement->iZ >= ptHeightMap->iSize - 1 ? ptHeightMap->atHaloElements[2 * (ptHeightMap->iSize - 1) + hx].fY : ptHeightMap->atElements[ptElement->iX  + ptHeightMap->iSize*jU].fY;
...
pl__update(plTerrainHeightMap* ptHeightMap, float fBaseMaxError, int iApexX, int iApexZ, int iRightX, int iRightZ, int iLeftX, int iLeftZ)
{
    ...
    {
        fError = tBaseVertex.y - (tLeftVertex.y + tRightVertex.y) / 2.0f;
    }
