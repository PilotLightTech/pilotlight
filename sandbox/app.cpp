...
pl_app_load(plApiRegistryI* ptApiRegistry, plAppData* ptAppData)
{
    ...
    tRenderSettings.ptWindow              = ptAppData->ptWindow;
    ...
    *ptAppData->pbShowDeviceMemoryAnalyzer = gptConfig->load_bool("pbShowDeviceMemoryAnalyzer", *ptAppData->pbShowDeviceMemoryAnalyzer);
    ...
    plDevice* ptDevice = gptRenderer->get_device();
        ...
        tImguiVulkanInfo.DescriptorPoolSize = 100;
        tImguiVulkanInfo.MinImageCount = 2;
        tImguiVulkanInfo.MSAASamples = (VkSampleCountFlagBits)gptGfx->get_swapchain_info(gptRenderer->get_swapchain()).tSampleCount;
        tImguiVulkanInfo.RenderPass = gptGfx->get_vulkan_render_pass(ptDevice, gptRenderer->get_main_render_pass());
        gptGfx->get_swapchain_images(gptRenderer->get_swapchain(), &tImguiVulkanInfo.ImageCount);
...
pl_app_update(plAppData* ptAppData)
{
    ...
    #elif defined(PL_METAL_BACKEND)
        ImGui_ImplMetal_NewFrame(gptGfx->get_metal_render_pass_descriptor(gptRenderer->get_device(), gptRenderer->get_main_render_pass()));
    #else
    #endif
