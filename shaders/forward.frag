...
{
    // return pl_srgb_to_linear(textureLod(sampler2D(at2DTextures[nonuniformEXT(tGpuScene.tData.iSheenLutIndex)], tSamplerLinearRepeat), vec2(NdotV, sheenRoughnessFactor), 0).r);
    // return pl_linear_to_srgb(textureLod(sampler2D(at2DTextures[nonuniformEXT(tGpuScene.tData.iSheenLutIndex)], tSamplerLinearRepeat), vec2(NdotV, sheenRoughnessFactor), 0).r);
    return textureLod(sampler2D(at2DTextures[nonuniformEXT(tGpuScene.tData.iSheenLutIndex)], tSamplerLinearRepeat), vec2(NdotV, sheenRoughnessFactor), 0).r;
}
