...
GLFWcursor*             MouseCursors[PL_MOUSE_CURSOR_COUNT];
...
void pl_glfw_mouse_button_callback(GLFWwindow* window, int button, int action, int mods);
void pl_glfw_mouse_pos_callback(GLFWwindow* window, double x, double y);
void pl_glfw_cursor_enter_callback(GLFWwindow* window, int entered);
void pl_glfw_window_focus_callback(GLFWwindow* window, int focused);
void pl_glfw_scroll_callback(GLFWwindow* window, double xoffset, double yoffset);
void pl_glfw_char_callback(GLFWwindow* window, unsigned int c);
void pl_glfw_key_callback(GLFWwindow* window, int keycode, int scancode, int action, int mods);
void pl_glfw_size_callback(GLFWwindow* window, int width, int height);
void pl_glfw_window_iconified_callback(GLFWwindow* window, int iconified);
void pl_glfw_window_close_callback(GLFWwindow* window);
...
int main(int argc, char *argv[])
{
    ...
    MouseCursors[PL_MOUSE_CURSOR_ARROW] = glfwCreateStandardCursor(GLFW_ARROW_CURSOR);
    MouseCursors[PL_MOUSE_CURSOR_TEXT_INPUT] = glfwCreateStandardCursor(GLFW_IBEAM_CURSOR);
    MouseCursors[PL_MOUSE_CURSOR_RESIZE_NS] = glfwCreateStandardCursor(GLFW_VRESIZE_CURSOR);
    MouseCursors[PL_MOUSE_CURSOR_RESIZE_EW] = glfwCreateStandardCursor(GLFW_HRESIZE_CURSOR);
    MouseCursors[PL_MOUSE_CURSOR_HAND] = glfwCreateStandardCursor(GLFW_HAND_CURSOR);
    #if GLFW_HAS_NEW_CURSORS
    MouseCursors[PL_MOUSE_CURSOR_RESIZE_ALL] = glfwCreateStandardCursor(GLFW_RESIZE_ALL_CURSOR);
    MouseCursors[PL_MOUSE_CURSOR_RESIZE_NESW] = glfwCreateStandardCursor(GLFW_RESIZE_NESW_CURSOR);
    MouseCursors[PL_MOUSE_CURSOR_RESIZE_NWSE] = glfwCreateStandardCursor(GLFW_RESIZE_NWSE_CURSOR);
    MouseCursors[PL_MOUSE_CURSOR_NOT_ALLOWED] = glfwCreateStandardCursor(GLFW_NOT_ALLOWED_CURSOR);
    #else
    ...
    while (gptIOCtx->bRunning)
    {
        ...
        plMouseCursor tCursor1 = ImGui::GetMouseCursor();

        if(tCursor0 != PL_MOUSE_CURSOR_ARROW)
        {
            glfwSetCursor(glfwWindow, MouseCursors[tCursor0] ? MouseCursors[tCursor0] : MouseCursors[PL_MOUSE_CURSOR_ARROW]);
...
pl_create_window(plWindowDesc tDesc, plWindow** pptWindowOut)
{
    ...
    glfwWindow = glfwCreateWindow((int)tDesc.uWidth, (int)tDesc.uHeight, tDesc.pcTitle, NULL, NULL);
    
    glfwSetMouseButtonCallback(glfwWindow, pl_glfw_mouse_button_callback);
    glfwSetCursorPosCallback(glfwWindow, pl_glfw_mouse_pos_callback);
    glfwSetCursorEnterCallback(glfwWindow, pl_glfw_cursor_enter_callback);
    glfwSetScrollCallback(glfwWindow, pl_glfw_scroll_callback);
    glfwSetCharCallback(glfwWindow, pl_glfw_char_callback);
    glfwSetKeyCallback(glfwWindow, pl_glfw_key_callback);
    glfwSetWindowFocusCallback(glfwWindow, pl_glfw_window_focus_callback);
    glfwSetWindowSizeCallback(glfwWindow, pl_glfw_size_callback);
    glfwSetWindowIconifyCallback(glfwWindow, pl_glfw_window_iconified_callback);
    glfwSetWindowCloseCallback(glfwWindow, pl_glfw_window_close_callback);
    ...
    #ifdef _WIN32
    HWND tHandle = glfwGetWin32Window(glfwWindow);
    ptWindow->_pPlatformData = tHandle;
    #elif defined(__APPLE__)
    device = MTLCreateSystemDefaultDevice();
    gptIOCtx->pBackendPlatformData = device;

    nswin = glfwGetCocoaWindow(glfwWindow);
    ...
    static tPlatformData tPlatformData = {UINT32_MAX};
    tPlatformData.dpy = glfwGetX11Display();
    tPlatformData.window = glfwGetX11Window(glfwWindow);
...
pl_glfw_size_callback(GLFWwindow* window, int width, int height)
{
    ...
    {
        int fwidth, fheight;
        glfwGetFramebufferSize(glfwWindow, &fwidth, &fheight);
...
pl_glfw_key_callback(GLFWwindow* window, int keycode, int scancode, int action, int mods)
{
    ...
    if (keycode >= 0 && keycode < PL_ARRAYSIZE(KeyOwnerWindows))
        KeyOwnerWindows[keycode] = (action == GLFW_PRESS) ? window : NULL;
