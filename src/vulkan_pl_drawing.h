...
// misc
VkDescriptorSet plg_add_texture(plDrawContext* drawContext, VkImageView imageView, VkImageLayout imageLayout);
...
pl_setup_draw_context_vulkan(plDrawContext* ctx, VkPhysicalDevice physicalDevice, uint32_t imageCount, VkDevice logicalDevice)
{
    ...
    PL_VULKAN(vkCreateImage(vulkanDrawContext->device, &imgInfo, NULL, &vulkanDrawContext->fontTextureImage));

    VkMemoryRequirements memReqs = {0};
    vkGetImageMemoryRequirements(vulkanDrawContext->device, vulkanDrawContext->fontTextureImage, &memReqs);

    VkMemoryAllocateInfo finalAllocInfo = {
        .sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
        .allocationSize = memReqs.size,
        .memoryTypeIndex = pl__find_memory_type(vulkanDrawContext->memProps, memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)
    };

    PL_VULKAN(vkAllocateMemory(vulkanDrawContext->device, &finalAllocInfo, NULL, &vulkanDrawContext->fontTextureMemory));
    PL_VULKAN(vkBindImageMemory(vulkanDrawContext->device, vulkanDrawContext->fontTextureImage, vulkanDrawContext->fontTextureMemory, 0));

    // upload data
    uint32_t dataSize = 2 * 2 * 4u;
    if(dataSize > vulkanDrawContext->stageByteSize)
    {
        if(vulkanDrawContext->stagingMemory)
        {
            vkUnmapMemory(vulkanDrawContext->device, vulkanDrawContext->stagingMemory);
            vkDestroyBuffer(vulkanDrawContext->device, vulkanDrawContext->stagingBuffer, NULL);
            vkFreeMemory(vulkanDrawContext->device, vulkanDrawContext->stagingMemory, NULL);
        }

        vulkanDrawContext->stageByteSize = dataSize*2;

        VkBufferCreateInfo stagingBufferInfo = {
            .sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
            .size = vulkanDrawContext->stageByteSize,
            .usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
            .sharingMode = VK_SHARING_MODE_EXCLUSIVE
        };

        PL_VULKAN(vkCreateBuffer(vulkanDrawContext->device, &stagingBufferInfo, NULL, &vulkanDrawContext->stagingBuffer));

        VkMemoryRequirements stagingMemRequirements = {0};
        vkGetBufferMemoryRequirements(vulkanDrawContext->device, vulkanDrawContext->stagingBuffer, &stagingMemRequirements);
        vulkanDrawContext->stageByteSize = stagingMemRequirements.size;

        VkMemoryAllocateInfo stagingAllocInfo = {
            .sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
            .allocationSize = stagingMemRequirements.size,
            .memoryTypeIndex = pl__find_memory_type(vulkanDrawContext->memProps, stagingMemRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)
        };

        PL_VULKAN(vkAllocateMemory(vulkanDrawContext->device, &stagingAllocInfo, NULL, &vulkanDrawContext->stagingMemory));
        PL_VULKAN(vkBindBufferMemory(vulkanDrawContext->device, vulkanDrawContext->stagingBuffer, vulkanDrawContext->stagingMemory, 0));   
        PL_VULKAN(vkMapMemory(vulkanDrawContext->device, vulkanDrawContext->stagingMemory, 0, VK_WHOLE_SIZE, 0, &vulkanDrawContext->mapping));
    }
    memcpy(vulkanDrawContext->mapping, image, dataSize);
    ...
    VkCommandBufferAllocateInfo allocInfo = {
        ...
        .commandPool = vulkanDrawContext->cmdPool,
        .commandBufferCount = 1u
    };
    VkCommandBuffer cmdBuffer = {0};
    vkAllocateCommandBuffers(vulkanDrawContext->device, &allocInfo, &cmdBuffer);
    ...
    VkImageMemoryBarrier barrier1 = {
        ...
        .image = vulkanDrawContext->fontTextureImage,
        .subresourceRange = subresourceRange,
        .srcAccessMask = 0,
        .dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT
    };
    vkCmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0, NULL, 0, NULL,1, &barrier1);
    ...
    vkCmdCopyBufferToImage(cmdBuffer, vulkanDrawContext->stagingBuffer, vulkanDrawContext->fontTextureImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);     
    ...
    vkCmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0, NULL, 0, NULL,1, &barrier2);
    ...
    vkQueueSubmit(vulkanDrawContext->graphicsQueue, 1, &submitInfo, VK_NULL_HANDLE);
    vkDeviceWaitIdle(vulkanDrawContext->device);
    vkFreeCommandBuffers(vulkanDrawContext->device, vulkanDrawContext->cmdPool, 1, &cmdBuffer);
    ...
    PL_VULKAN(vkCreateImageView(vulkanDrawContext->device, &viewInfo, NULL, &vulkanDrawContext->fontTextureImageView));
    ctx->fontAtlas = plg_add_texture(ctx,vulkanDrawContext->fontTextureImageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
}
...
VkDescriptorSet
plg_add_texture(plDrawContext* drawContext, VkImageView imageView, VkImageLayout imageLayout)
{
    plVulkanDrawContext* vulkanData = (plVulkanDrawContext*)drawContext->_platformData;
